...
class stop():
    ...
    def get_link_data(self, stop_id, link):
    ...
    def get_stop_links(self, next_stop):
        try:
            next_stop_links = [i.split('.')[0] for i in \
                            os.listdir('/home/student/data/stops/'+str(next_stop))\
                            if i != 'orphans.csv']      
            return next_stop_links
        except:
            print("Missing stop:")
            return
    
    def get_all_data(self):   #Trying to get data for every stop, that is wihtin 4 stops of this stop class. This could be the worst code ever written like o(n^4) but I cant think of a better way. Maybe change to using different data structures.
        """
        Retrieve all data about this stop
        """
        to_concat = []
        furthest_links = [] #Used to store links that are the furthest possible away from stop (max of 4).
        import pandas as pd
        for link in self.stop_links:
            isFurthest = link
            to_concat.append(self.get_link_data(self.stop_id, link))
            stop_links_depth1 = self.get_stop_links(link) #QUESTION OVER INCLUDING CURRENT STOP IN NEW STOP LINKS
            if stop_links_depth1 is None:
                furthest_links.append(isFurthest)
            else:
                for next_link1 in stop_links_depth1:
                    isFurthest = next_link1
                    to_concat.append(self.get_link_data(link, next_link1))
                    stop_links_depth2 = self.get_stop_links(next_link1) #QUESTION OVER INCLUDING CURRENT STOP IN NEW STOP LINKS
                    if stop_links_depth2 is None:
                        furthest_links.append(isFurthest)
                    else:
                        for next_link2 in stop_links_depth2:
                            isFurthest = next_link2
                            to_concat.append(self.get_link_data(next_link1, next_link2))
                            stop_links_depth3 = self.get_stop_links(next_link2) #QUESTION OVER INCLUDING CURRENT STOP IN NEW STOP LINKS
                            if stop_links_depth3 is None:
                                furthest_links.append(isFurthest)
                            else:
                                for next_link3 in stop_links_depth3:
                                    isFurthest = next_link3
                                    to_concat.append(self.get_link_data(next_link2, next_link3))
                                    furthest_links.append(isFurthest)
                
        return (self.prep_data(pd.concat(to_concat, axis=0)), furthest_links)
    ...
    def train_models(self,split=None):
        ...
        data, links = self.get_all_data() #Get dataframe and furthest stops
        self.linkmodels = {}
        for link in links:
            in_data = data[data['stopB']==link]     #I think this is correct, but there is potenially a error here.        
